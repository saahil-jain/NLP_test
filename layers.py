...
def attention(query, key, value, mask=None, dropout=None):
    ...
    scaled_QK = QK / math.sqrt(query.size(-1))

    if mask is not None:
        mask = mask.unsqueeze(1)
        scaled_QK = scaled_QK.masked_fill(mask == 0, -1e9)
    soft = nn.functional.softmax(scaled_QK, dim=-1)
    if dropout is not None:
        soft = dropout(soft)
    Z = torch.matmul(soft, value)
    return Z, soft
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        Z, self.attn = attention(query, key, value, mask=mask, dropout=self.dropout)

        del query
        del key
        del value

        Z = (Z.transpose(1, 2).contiguous().view(N, -1, self.h * self.d_k))
